...
export class UserFixture {
   ...
   */
  static async createAndSaveUser(overrides: Partial<User> = {}): Promise<User> {
  ...
  static createUsers(count: number, overrides: Partial<User> = {}): User[] {
    return Array.from({ length: count }, (_, index) =>
      UserFixture.createUser({
        ...overrides,
        email: overrides.email ? `${index + 1}_${overrides.email}` : `test${index + 1}@example.com`,
      })
    );
  }
   ...
   */
  static async createAndSaveUsers(count: number, overrides: Partial<User> = {}): Promise<User[]> {
   ...
   */
  static async createAndSaveAdminUser(overrides: Partial<User> = {}): Promise<User> {
